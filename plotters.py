...
def getfromout_hist(out):
    zs = out[0]
    xs = out[1]
    ys = out[2]
    return(xs,ys,zs)
  
def getfromout_hex(out):
    zs = out.get_array()
    xs = out.get_offsets().T[0]
    ys = out.get_offsets().T[1]
    return(xs,ys,zs)

def overlaycontour(f,ax,out,c='k',lvls=5,lw=2,rtarr=0,hex=True,hist=False):
    if hex:
    ...
    try:
        ax.tricontour(xs,ys,zs,levels=lvls,colors=c,linewidths=lw)
    except ValueError:
        ax.tricontour(xs,ys,zs,levels=lvls,cmap=c,linewidths=lw)
    if rtarr:
        return(f,ax,out)
    else:
        return(f,ax)
