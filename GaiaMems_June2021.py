...
class GaiaClusterMembers(object):
    ...
    def getPMMembers(self,cmarkers=None):
        ...
        #bins ## bins[0] must be positive, when an integer
        ...
        if self.weightedG == True:
            hx1D, x1D = np.histogram(xm, bins=pmRAbins, weights=xmW)
            hy1D, y1D = np.histogram(ym, bins=pmDecbins, weights=ymW)
            
            #2D histogram on the members so far
            h2D, x2D, y2D = np.histogram2d(xm, ym, bins=[self.PMxbins, self.PMybins], \
                                           range=[[self.PMxmin, self.PMxmax], [self.PMymin, self.PMymax]], weights=mW)
        else:
